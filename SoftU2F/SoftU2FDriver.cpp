...
void SoftU2FDriverClassName::stop(IOService* provider)
{
    ...
    if (iter) {
        ...
        while ((key = (char*)iter->getNextObject())) {
            SoftU2FDeviceClassName *device = (SoftU2FDeviceClassName *)m_hid_devices->getObject(key);
...
IOService* SoftU2FDriverClassName::userClientDevice(IOService* userClient) {
    ...
    if (!device) {
        ...
        if (!device) goto fail;
        if (!device->init(nullptr)) goto fail;
        if (!m_hid_devices->setObject(key, device)) goto fail;
